...
<cfif isdefined('submit')>
	...
	<cfquery name="insertMSTR" datasource="BusinessPLUS">
		INSERT INTO Stipend_PP_MSTR	(tbl_Index, setid, username, building, principal, enteredon, principal_app_deny, hr_app_deny, payroll_status, submitted, FiscalYR)
	...
	<cflocation url="PlanningPeriod.cfm">
</cfif>
...
<html lang="en">
  ...
  <head>
    ...
    <title>Stipend - New Planning Period</title>
...
<body>	
	<!--- new set id --->
	<!--- OLD keep for now <cfset session.StipendSet =  '#session.building.code#'&'#dateformat(NOW(), "mmddyy")#'>--->
	...
	<cfset session.StipendSet = 'PP'&'#NUMBERFORMAT(GetNextSetID.NewID, "000009")#'>
	...
	<cfform name="pp_stipend_mstr" action="new_ppstipend.cfm" method="post">
	<center>
		<table id="new-pp-stipend-table">
			<th id="new-pp-stipend-th" colspan="2">NEW - REQUEST FOR PLANNING PERIOD COMPENSATION WHEN<br>
